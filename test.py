...
def main():
    ...
    args.load = ospj(logpath, 'ckpt_best_auc.t7')  # {'ckpt_best_hm.t7'}
    ...
    with torch.no_grad():
        test(image_extractor, model, testloader, evaluator, args, gfn= gfn)


def test(image_extractor, model, testloader, evaluator, args, print_results=True, gfn=None):
        if image_extractor:
            image_extractor.eval()
        if gfn:
            gfn.eval()
