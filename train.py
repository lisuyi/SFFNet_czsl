...
def main():
    ...
    image_extractor, gfn, model, optimizer = configure_model(args, trainset)
    ...
    for epoch in tqdm(range(start_epoch, args.max_epochs + 1), desc='Current epoch---'):
        train(epoch, image_extractor, model, trainloader, optimizer, writer, gfn=gfn)
        if epoch % args.eval_val_every == 0:
            with torch.no_grad():
                test(epoch, image_extractor, model, testloader, evaluator_val, writer, args, logpath, gfn=gfn)
...
def train_normal(epoch, image_extractor, model, trainloader, optimizer, writer, gfn=None):
    if image_extractor:
...
def test(epoch, image_extractor, model, testloader, evaluator, writer, args, logpath, gfn=None):
    ...
    for idx, data in tqdm(enumerate(testloader), total=len(testloader), desc='Testing'):
        ...
        _, _, _, predictions, pred_attr, pred_obj = model(data)
